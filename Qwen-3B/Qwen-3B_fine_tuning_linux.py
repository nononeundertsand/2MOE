# train_linux.py
"""
Single-GPU Linux-compatible training script for Qwen2.5-3B-Instruct with LoRA (PEFT).
Usage: python train_linux.py
Requirements:
...
import os
# Linux环境下使用官方HF端点（国内服务器可能需要镜像，根据实际情况选择）
# os.environ["HF_ENDPOINT"] = "https://hf-mirror.com"  # 如果需要镜像取消注释
...
# Linux下可以启用torch.compile加速（可选，需要PyTorch 2.0+）
...
tokenized = raw_ds.map(tokenize_example, remove_columns=raw_ds.column_names, num_proc=os.cpu_count())
...
training_args = TrainingArguments(
    ...
    dataloader_num_workers=4,  # Linux下可以设置多进程数据加载
    save_total_limit=2,         # 只保留最后2个checkpoint
    load_best_model_at_end=False,  # 是否加载最佳模型
)
