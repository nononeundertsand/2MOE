...
本文档用于指导完整实现 2MoE 系统，包括数据准备、聚类、专家构建、量化、Gating 网络训练、MoE 推理、SMO 子集选择、以及系统级延迟/能耗评测。

---

# Overview

构建一个面向异构边缘设备的多专家（Mixture-of-Experts）LLM 推理系统。
主要内容包含：
...
使用 mGTE encoder，或者使用替代方案（如 sentence-transformers）。
...
全部使用 Q4 量化，使用 GPTQ/AWQ/bitsandbytes 均可：
